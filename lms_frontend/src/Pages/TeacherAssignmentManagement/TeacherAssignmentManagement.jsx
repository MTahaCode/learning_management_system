...
import "../../css/TeacherAssignmentManagement.css";

const TeacherAssignmentManagement = () => {
  const [courseList, setCourseList] = useState([]);
  const [potentialAssignmentTeachers, setPotentialAssignmentTeachers] =
    useState([]);
  ...
  const getPotentialAssignmentTeachers = () => {
    fetch(
      `/teacher/unassigned/course/${selectedCourse.course_id}/section/${selectedSection}`
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  const getSectionsOfSelectedCourse = () => {
    fetch(
      `/teacherAssignment/avaliableSectionsOfCourse/${selectedCourse.course_id}`
    )
      .then((res) => res.json())
      .then((data) => {
        setAvaliableSections(
          sectionOptions.filter((option) => !data.includes(option.value))
        );
      });
  };
  ...
  return (
    <div className="teacher-assignment-container">
      ...
      <div className="teacher-list">
        {potentialAssignmentTeachers.length > 0 &&
          potentialAssignmentTeachers.map((row) => {
            return (
      ...
      <h1>Selection</h1>
      <p>
        Selected Course: {selectedCourse.course_id} {selectedCourse.course_name}
      </p>
      <p>
        Selected Teacher: {selectedTeacher.teacher_id}{" "}
